           ...
           <div key={skill.id} className={`border-2 p-2 transition-all flex flex-col md:flex-row gap-2 relative ${level > 0 ? 'border-cyan-600 bg-cyan-950/10' : 'border-emerald-950 opacity-80'}`}>
              <div className="flex-1 min-w-0">
                 <div className="flex items-center gap-2">
                    <span className={`text-xs md:text-base font-black uppercase ${level > 0 ? 'text-white' : reqMet ? 'text-emerald-500' : 'text-emerald-900'}`}>{skill.name}</span>
                    <span className={`text-[10px] px-1 border uppercase ${isPassive ? 'border-purple-900 text-purple-400' : 'border-cyan-900 text-cyan-600'}`}>{skill.type}</span>
                    {!reqMet && <span className="text-[10px] text-red-500 font-bold border border-red-900 px-1 bg-red-950/30">REQ LVL {skill.minLevel}</span>}
                 </div>
                 <p className="text-[10px] md:text-sm text-cyan-800 leading-tight italic mt-0.5">"{skill.desc}"</p>
                 {level > 0 && !isPassive && (
                    <div className="text-[10px] text-cyan-700/60 uppercase mt-1">
                       Power: {powerMult.toFixed(2)}x | Cost: {skill.cost} MP
                    </div>
                 )}
                 {level > 0 && isPassive && skill.passiveStat && (
                    <div className="text-[10px] text-purple-700/60 uppercase mt-1">
              ...
              <div className="flex flex-row md:flex-col items-end justify-between shrink-0 gap-2">
                 ...
                 {!isMax ? (
                    <button 
                      ...
                      className={`px-3 py-1 text-[10px] md:text-sm font-black border transition-all w-24
                        ${!reqMet 
                    ...
                    <span className="text-[10px] md:text-sm font-black text-cyan-400 uppercase tracking-widest w-24 text-right">MAX</span>
                 )}
              </div>
           </div>
         );
  ...
  return (
    <div className="flex flex-col h-full bg-black text-emerald-500 font-mono p-4 animate-in fade-in zoom-in duration-300 border-4 border-cyan-900 shadow-[0_0_50px_rgba(0,0,0,0.9)] overflow-hidden">
      <div className="flex justify-between items-center border-b-2 border-cyan-900 pb-3 mb-4 shrink-0">
        <h2 className="text-2xl md:text-3xl font-black tracking-[0.2em] uppercase text-cyan-400">Skill Sanctum</h2>
        <button onClick={onClose} className="retro-button px-3 py-1 text-xs md:text-sm border-red-900 text-red-500 hover:bg-red-900 hover:text-white">EXIT</button>
      </div>
      ...
      <div className="flex gap-1 mb-4 shrink-0">
        {party.map((p, i) => (
          <button 
            ...
            className={`flex-1 py-1.5 border-2 text-[10px] md:text-sm font-black transition-all flex items-center justify-center gap-2 ${
      ...
      <div className="bg-cyan-950/20 border border-cyan-900/50 p-3 mb-4 flex justify-between items-center rounded-sm shrink-0">
           <div className="flex items-center gap-4">
              ...
              <div>
                  <div className="text-xs md:text-lg font-black text-white">{player.class}</div>
                  <div className="text-[10px] md:text-sm text-cyan-700">Level {player.level}</div>
              </div>
           </div>
           <div className="text-right">
              <div className="text-base md:text-2xl font-black text-cyan-400">{player.skillPoints} SP</div>
              <div className="text-[10px] md:text-xs text-cyan-900 uppercase">Available</div>
           </div>
      </div>

      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
         <div className="mb-4">
            <h3 className="text-lg font-black text-cyan-600 border-b border-cyan-900/50 pb-1 mb-2 tracking-widest">ACTIVE SKILLS</h3>
            {renderSkillList(actives)}
         </div>
         <div>
            <h3 className="text-lg font-black text-purple-600 border-b border-purple-900/50 pb-1 mb-2 tracking-widest">PASSIVE TRAITS</h3>
